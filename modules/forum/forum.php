...
class m_forum extends Module
{
	...
	public static function permissions()
	{
		return [
			'category' => [
				'get_all' => function(){
					return Dungeon()->db->select('category_id', 'CONCAT_WS(" ", "{lang category}", title)')->from('dungeon_forum_categories')->get();
				},
				'check'   => function($category_id){
					if (($category = Dungeon()->db->select('title')->from('dungeon_forum_categories')->where('category_id', $category_id)->row()) !== [])
					{
		...
		if (!isset($profiles[$user_id]))
		{
			...
			else
			{
				$profiles[$user_id]['topics'] = $this->db	->select('COUNT(*)')
															...
															->join('dungeon_forum_messages m', 't.message_id = m.message_id')
															->where('m.user_id', $user_id)
															->row();
				
				$profiles[$user_id]['replies'] = $this->db->select('COUNT(*)')->from('dungeon_forum_messages')->where('user_id', $user_id)->row() - $profiles[$user_id]['topics'];
			}
		}
