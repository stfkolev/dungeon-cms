...
class m_forum_m_forum extends Model
{
	public function get_categories_list($forum_id = NULL)
	{
		...
		foreach ($this->db	->select('c.category_id', 'c.title', 'f.forum_id', 'f.title as forum_title')
							->from('dungeon_forum_categories c')
							->join('dungeon_forum f', 'c.category_id = f.parent_id AND f.is_subforum = "0"')
							...
							->get() as $category)
		{
			if ($this->access('forum', 'category_read', $category['category_id']))
			{
				...
				foreach ($this->db	->select('f.forum_id', 'f.title')
									...
									->join('dungeon_forum_url u', 'u.forum_id = f.forum_id')
	...
	public function get_forums($forum_id = NULL, $mini = FALSE)
	{
		...
		{
			$this->db	->join('dungeon_forum f2', 'f2.parent_id = f.forum_id AND f2.is_subforum = "1"')
						->where('f.is_subforum', FALSE);
		}
		...
		$forums = $this->db	->select(	'f.forum_id',
									...
									->from('dungeon_forum f')
									->join('dungeon_forum_messages m', 'm.message_id = f.last_message_id')
									->join('dungeon_forum_topics t',   't.topic_id = m.topic_id')
									->join('dungeon_users u',          'u.user_id = m.user_id AND u.deleted = "0"')
									->join('dungeon_forum_url u2',     'u2.forum_id = f.forum_id')
									->group_by('f.forum_id')
									->order_by('f.order', 'f.forum_id')
									->get();
	...
	public function get_topics($forum_id)
	{
		$topics = $this->db->select('t.topic_id',
						...
						->from('dungeon_forum_topics   t')
						->join('dungeon_forum_messages m1', 't.message_id = m1.message_id')
						->join('dungeon_forum_messages m2', 't.last_message_id = m2.message_id')
						->join('dungeon_users u1',          'u1.user_id = m1.user_id AND u1.deleted = "0"')
						->join('dungeon_users u2',          'u2.user_id = m2.user_id AND u2.deleted = "0"')
		...
		if ($this->user())
		{
			...
			foreach ($this->db->select('t.topic_id', 'r.date')
									->from('dungeon_forum_topics_read r')
									->join('dungeon_forum_topics t', 't.topic_id = r.topic_id')
									->where('t.forum_id', $forum_id)
	...
	public function check_forum($forum_id, &$title)
	{
		$forum = $this->db	->select('f.forum_id', 'f.title', 'f.description', 'f.parent_id', 'f.is_subforum', 'u.url', 'IFNULL(f3.parent_id, f.parent_id) as category_id', 'COUNT(f2.forum_id) as subforums')
							->from('dungeon_forum f')
							->join('dungeon_forum f2', 'f2.parent_id = f.forum_id  AND f2.is_subforum = "1"')
							->join('dungeon_forum f3', 'f3.forum_id  = f.parent_id AND f.is_subforum  = "1"')
							->join('dungeon_forum_url u', 'u.forum_id = f.forum_id')
	...
	public function add_topic($forum_id, $title, $message, $announce)
	{
		$topic_id = $this->db	->ignore_foreign_keys()
								->insert('dungeon_forum_topics', [
									'forum_id' => (int)$forum_id,
									'title'    => $title,
									'status'   => $announce
								]);
		
		$message_id = $this->db	->insert('dungeon_forum_messages', [
									'topic_id' => $topic_id,
									'user_id'  => $this->user('user_id'),
									'message'  => $message
								]);

		$count_topics = $this->db->select('count_topics')->from('dungeon_forum')->where('forum_id', $forum_id)->row();
	...
	{
		$topic = $this->db->select('count_messages', 'forum_id')->from('dungeon_forum_topics')->where('topic_id', $topic_id)->row();
		$count_messages = $this->db->select('count_messages')->from('dungeon_forum')->where('forum_id', $topic['forum_id'])->row();
	...
	{
		$category_id = $this->db->insert('dungeon_forum_categories', [
			'title' => $title
		]);
	...
	{
		$forum_id = $this->db->insert('dungeon_forum', [
			'title'       => $title,
	...
	{
		$this->db	->where('category_id', $category_id)
					->delete('dungeon_forum_categories');
					
		foreach ($this->db->select('forum_id')->from('dungeon_forum')->where('parent_id', $category_id)->where('is_subforum', FALSE)->get() as $forum_id)
		{
	...
	{
		foreach ($this->db->select('forum_id')->from('dungeon_forum')->where('parent_id', $forum_id)->where('is_subforum', TRUE)->get() as $subforum_id)
		{
			$this->delete_forum($subforum_id);
		}

		$this->db	->where('forum_id', $forum_id)
					->delete('dungeon_forum');
					
		$this->db	->where('forum_id', $forum_id)
					->delete('dungeon_forum_read');
	}
	...
	public function mark_all_as_read($forum_id = 0)
	{
		...
		if ($forum_id)
		{
			$this->db	->where('r.user_id', $this->user('user_id'))
						...
						->where('t.forum_id', $forum_id)
						->delete('r', 'dungeon_forum_topics_read as r, dungeon_forum_topics as t');
		...
		$this->db->insert('dungeon_forum_read', [
			'user_id'  => $this->user('user_id'),
			'forum_id' => $forum_id,
		]);
	}
	...
	{
		$this->db	->where('forum_id', $forum_id)
					->update('dungeon_forum_url', 'redirects = redirects + 1');
	}
