		...
		<div class="text-left col-xs-<?php echo $data['match']['opponent']['image_id'] ? 4 : 5; ?> vcenter">
			<h3 class="no-margin">
				<?php
					...
					{
						$opponent .= '<img src="'.url('dungeon/themes/default/images/flags/'.$data['match']['opponent']['country'].'.png').'" data-toggle="tooltip" title="'.get_countries()[$data['match']['opponent']['country']].'" style="margin-left: 10px;" alt="" />';
					}
...
<div class="panel-footer">
	...
	<ul class="list-inline no-margin">
		...
		<li><?php echo icon('fa-clock-o'); ?> <?php echo '<span data-toggle="tooltip" title="'.timetostr(Dungeon()->lang('date_time_long'), $data['date']).'">'.timetostr(Dungeon()->lang('date_time_short'), $data['date']).'</span>'.($data['date_end'] ? '&nbsp;&nbsp;<span data-toggle="tooltip" title="Durée"><i>'.icon('fa-hourglass-end').(ceil((strtotime($data['date_end']) - strtotime($data['date'])) / ( 60 * 60 ))).'h</i></span>' : ''); ?></li>
		<?php
