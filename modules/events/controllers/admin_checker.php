...
class m_events_c_admin_checker extends Controller_Module
{
	...
	public function _round_delete($event_id, $title, $round_id)
	{
		...
		if ($this->model()->check_event($event_id, $title) && $this->db->select('round_id')->from('dungeon_events_matches_rounds')->where('round_id', $round_id)->where('event_id', $event_id)->row())
		{
			return [$round_id];
		}
	}
