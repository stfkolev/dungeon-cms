	...
	<div class="row">
		<?php foreach ($data['gallery'] as $album): ?>
			<div class="col-md-12">
				...
				<a href="<?php echo url('gallery/album/'.$album['gallery_id'].'/'.url_title($album['title'])); ?>" class="gallery-item thumbnail" style="background-image: url('<?php echo path(!empty($album['image']) ? $album['image'] : $this->db->select('file_id')->from('dungeon_gallery_images')->where('gallery_id', $album['gallery_id'])->order_by('RAND()')->limit(1)->row()); ?>');">
