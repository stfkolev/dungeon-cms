...
class m_games_m_modes extends Model
{
	public function get_modes($game_id = NULL)
	{
		...
		return $this->db->from('dungeon_games_modes')
						->order_by('title')
						->get();
	}
	...
	public function get_modes_list()
	{
		...
		foreach ($this->db	->select('m.mode_id', 'm.title', 'gl.title as game_title', 'gl2.title as game_title2')
							->from('dungeon_games_modes m')
							->join('dungeon_games g',        'm.game_id = g.game_id')
							->join('dungeon_games_lang gl',  'm.game_id = gl.game_id')
							->join('dungeon_games_lang gl2', 'g.parent_id = gl2.game_id')
	...
	{
		$mode = $this->db->from('dungeon_games_modes')
	...
	{
		return $this->db->insert('dungeon_games_modes', [
			'game_id' => $game_id,
			'title'   => $title
		]);
	}
