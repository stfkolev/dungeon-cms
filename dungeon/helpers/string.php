...
function strtolink($string, $is_html = FALSE)
{
	...
	return preg_replace_callback('_@((?:&quot;(.+?)&quot;)|([^@\s]+))_', function($match){
		...
		if ($users === NULL)
		{
			foreach (Dungeon()->db->select('user_id', 'username')->from('dungeon_users')->where('deleted', FALSE)->get() as $user)
			{
		...
		return ($user_id = array_search($username, $users)) !== FALSE ? Dungeon()->user->link($user_id, $username, '@') : $match[0];
	}, $string);
}
...
{
	return nl2br(strtolink(Dungeon()->bbcode->bbcode2html($string), TRUE));
}
