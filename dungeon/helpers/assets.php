...
function asset($file_path, $file_name = '')
{
	...
	if (!isset($content))
	{
		foreach (Dungeon()->paths('assets') as $path)
		{
	...
	if (isset($content))
	{
		...
		{
			$data = [
				'lang' => Dungeon()->config->lang
			];

			$content = Dungeon()->view->content($content, $data);
		}
...
function path($file, $file_type = '', $paths = [])
{
	if (func_num_args() == 1)
	{
		...
		if (!isset($paths[$file]))
		{
			$paths[$file] = Dungeon()->db->select('path')
												->from('dungeon_files')
												->where('file_id', $file)
												->row();
		}
	...
	else
	{
		...
		if (!$paths)
		{
			$loader = Dungeon()->theme ? Dungeon()->theme->load : Dungeon();
