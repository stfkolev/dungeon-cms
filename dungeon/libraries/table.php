	...
	public function display()
	{
		...
		if ($this->_is_searchable())
		{
			if ($search)
			{
				...
				$this->_data    = $results;
				$this->_no_data = Dungeon()->lang('no_result');
			}
			else
			{
		...
		if (empty($this->_data))
		{
			$output = '<div class="clearfix"></div>'.($this->_no_data ?: Dungeon()->lang('no_data'));
		}
		else
		{
			if (!$this->_ajax && $this->_is_searchable())
			{
				$search_input = '	<div class="table-search pull-left">
										<div class="form-group has-feedback">
											<input class="form-control" data-provide="typeahead" data-items="5" data-source="'.utf8_htmlentities('['.implode(', ', array_unique(array_filter($words))).']').'" type="text"'.(!empty($search) ? ' value="'.$search.'"' : '').' placeholder="'.Dungeon()->lang('search').'" autocomplete="off" />
										</div>
									</div>';
			}
			...
			if ($this->_pagination && !empty($this->pagination) && $this->pagination->count() > 10)
			{
				$output .= '<div class="form-group pull-left">
								<select class="form-control" style="width: auto;" onchange="window.location=\''.$this->pagination->get_url().'/\'+$(this).find(\'option:selected\').data(\'url\')" autocomplete="off">
									<option value="10"'. ($this->pagination->get_items_per_page() == 10  ? ' selected="selected"' : '').' data-url="page/1/10">'.Dungeon()->lang('results', 10, 10).'</option>
									<option value="25"'. ($this->pagination->get_items_per_page() == 25  ? ' selected="selected"' : '').' data-url="page/1/25">'.Dungeon()->lang('results', 25, 25).'</option>
									<option value="50"'. ($this->pagination->get_items_per_page() == 50  ? ' selected="selected"' : '').' data-url="page/1/50">'.Dungeon()->lang('results', 50, 50).'</option>
									<option value="100"'.($this->pagination->get_items_per_page() == 100 ? ' selected="selected"' : '').' data-url="page/1/100">'.Dungeon()->lang('results', 100, 100).'</option>
									<option value="all"'.($this->pagination->get_items_per_page() == 0   ? ' selected="selected"' : '').' data-url="all">'.Dungeon()->lang('show_all').'</option>
								</select>
							</div>';
			}
			...
			$output .= '<i>'.Dungeon()->lang('results', $count, $count).($count < $count_results ? Dungeon()->lang('results_total', $count_results) : '').'</i>';
