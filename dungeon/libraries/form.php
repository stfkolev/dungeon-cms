...
class Form extends Library
{
	...
	static private function _token($id)
	{
		...
		if ($tokens === NULL)
		{
			$tokens = Dungeon()->session('form') ?: [];
		}

		if (empty($tokens[$id]))
		{
			Dungeon()->session->set('form', $id, $tokens[$id] = unique_id(array_merge([$id], $tokens)));
		}
				...
				foreach ($this->_rules as $var => $options)
				{
					if (isset($options['type']) && $options['type'] == 'file')
					{
						...
						if (!empty($files['tmp_name'][$var]))
						{
							...
							{
								$this->_errors[$var] = Dungeon()->lang('file_transfer_error');
								return FALSE;
							}
	...
	private function _check_text($post, $var, $options)
	{
		...
		{
			return Dungeon()->lang('invalid_values', count($post[$var]));
		}
		...
		{
			return Dungeon()->lang('required_input');
		}
		
		if ($is_file && !empty($_FILES[$this->token()]['error'][$var]) && $_FILES[$this->token()]['error'][$var] != 4)
		{
			return Dungeon()->lang('file_transfer_error_'.$_FILES[$this->token()]['error'][$var]);
		}
	...
	private function _check_email($post, $var, $options)
	{
		...
		{
			return Dungeon()->lang('wrong_email');
		}
	...
	public function display()
	{
		if ($this->_confirm_deletion)
		{
			...
			if ($this->url->ajax())
			{
				return '<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">'.Dungeon()->lang('close').'</span></button>
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-default" data-dismiss="modal">'.Dungeon()->lang('cancel').'</button>
							<a class="btn btn-danger delete-confirm" href="'.url($this->url->request).'" data-form-id="'.$this->token().'" onclick="return confirm_deletion(this);">'.Dungeon()->lang('remove').'</a>
						</div>';
			}
			else
			{
		...
		{
			Dungeon()->js('https://www.google.com/recaptcha/api.js?hl='.$this->config->lang.'&_=');
		...
		{
			$output .= '<div class="form-group"><div class="col-md-offset-3 col-md-9"><em class="text-muted">'.Dungeon()->lang('required_fields').'</em></div></div>';
		}
	...
	private function _display_text($var, $options, $post, $type = 'text')
	{
		...
		if (in_array($type, ['date', 'datetime', 'time']))
		{
			...
			Dungeon()	->css('bootstrap-datetimepicker.min')
		...
		else if ($type == 'colorpicker')
		{
			...
			Dungeon()	->css('bootstrap-colorpicker.min')
		...
		if ($type == 'file')
		{
			...
			$input = '<div style="margin: 7px 0;"><p>'.icon('fa-download').' '.Dungeon()->lang('upload_file').(!empty($options['info']) ? $options['info'] : '').'</p>'.$input.'</div>';
					...
					$input = '	<div class="row">
									<div class="col-md-3">
										<div class="thumbnail no-margin">
											<img src="'.url($this->db->select('path')->from('dungeon_files')->where('file_id', $options['value'])->row()).'" alt="" />
											<div class="caption text-center">
												<a class="btn btn-outline btn-danger btn-xs form-file-delete" href="#" data-input="'.$this->token().'['.$var.']">'.icon('fa-trash-o').' '.Dungeon()->lang('remove').'</a>
											</div>
										</div>
									</div>
									<div class="col-md-9">
	...
	{
		Dungeon()	->css('bootstrap-iconpicker.min')
							...
							->js_load('	$(".btn.iconpicker").iconpicker({
											...
											rows: 5,
											iconset: "fontawesome",
											labelHeader: "'.Dungeon()->lang('pages').'",
											labelFooter: "<div class=\"pull-right\">'.Dungeon()->lang('icons').'</div>",
											searchText: "'.Dungeon()->lang('search...').'",
	...
	private function _display_date($var, $options, $post)
	{
		...
		{
			$options['value'] = timetostr(Dungeon()->lang('date_short'), $options['value']);
		}
		else
		{
			$options['value'] = '';
	...
	private function _display_datetime($var, $options, $post)
	{
		if (isset($options['value']) && $options['value'] !== '')
		{
			$options['value'] = timetostr(Dungeon()->lang('date_time_short'), $options['value']);
		}
		else
		{
			$options['value'] = '';
	...
	private function _display_time($var, $options, $post)
	{
		...
		{
			$options['value'] = timetostr(Dungeon()->lang('time_short'), $options['value']);
		}
		else
		{
			$options['value'] = '';
