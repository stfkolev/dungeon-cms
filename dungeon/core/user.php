...
class User extends Core
{
	...
	public function __construct()
	{
		...
		if ($this->config->dungeon_http_authentication && $this->session('user_id') === NULL && $this->session('session', 'http_authentication'))
		{
	...
	private function _init()
	{
		if ($user_id = $this->session('user_id'))
		{
			$user = $this->db	->select('u.username', 'u.password', 'u.salt', 'u.email', 'u.admin', 'u.language', 'u.registration_date', 'p.first_name', 'p.last_name', 'p.avatar', 'p.signature', 'p.date_of_birth', 'p.sex', 'p.location', 'p.website', 'p.quote')
								...
								->join('dungeon_users_profiles p', 'u.user_id = p.user_id')
		...
		if ($this->config->dungeon_maintenance)
		{
			if ($this->config->dungeon_maintenance_opening && date_create() >= ($opening = date_create($this->config->dungeon_maintenance_opening)))
			{
				$this	->config('dungeon_maintenance', FALSE, 'bool')
						->config('dungeon_maintenance_opening', '');
			}
				...
				if (!$this->url->ajax_header)
				{
					...
					echo $theme->view('default', [
						'body'       => $theme->view('maintenance', [
							'page_title' => $page_title = $this->config->dungeon_maintenance_title ?: Dungeon()->lang('website_under_maintenance')
						]),
						'lang'       => $this->config->lang,
						'css'        => output('css'),
						'js'         => output('js'),
						'js_load'    => output('js_load'),
						'page_title' => $page_title.' :: '.$this->config->dungeon_name
					]);
				}
	...
	{
		$sessions =  $this->db	->from('dungeon_sessions')
	...
	{
		return $this->db->from('dungeon_sessions_history')
	...
	public function get_messages()
	{
		...
		if ($count === NULL)
		{
		 $count= $this->db	->select('COUNT(*)')
							...
							->join('dungeon_users_messages            m',   'r.message_id = m.message_id',    'INNER')
							->join('dungeon_users_messages_replies    mr',  'mr.reply_id  = m.last_reply_id', 'INNER')
							->join('dungeon_users_messages_replies    mr2', 'mr2.reply_id  = m.reply_id',     'INNER')
	...
	{
		if (!$this() && $this->config->dungeon_http_authentication && $this->session('session', 'http_authentication') !== TRUE)
		{
			$this->session->set('session', 'http_authentication', TRUE);

			header('WWW-Authenticate: Basic realm="'.$this->config->dungeon_http_authentication_name.'", encoding="UTF-8"');
	...
	public function link($user_id = 0, $username = '', $prefix = '')
	{
		...
		if (!$username)
		{
			$username = $this->db->select('username')->from('dungeon_users')->where('user_id', $user_id)->row();
		}
