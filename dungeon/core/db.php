...
class Db extends Core
{
	...
	private function _connect($config)
	{
		...
		{
			$this->debug->log('Fichier de configuration mal formé : ./dungeon/config/db.php', Debug::WARNING);
			return;
		}

		if (check_file($path = 'dungeon/databases/'.$config['driver'].'.php'))
		{
			require_once $path;
		...
		{
			$this->debug->log('Pilote de base de données introuvable : ./dungeon/databases/'.$config['driver'].'.php', Debug::WARNING);
		}
	}
