...
class Session extends Core
{
	...
	public function __construct()
	{
		...
		$this->db	->where('remember_me', FALSE)
					->where('UNIX_TIMESTAMP(last_activity) <', time() - strtoseconds($this->config->dungeon_cookie_expire))
		...
		if (isset($_COOKIE[$this->config->dungeon_cookie_name]) && $this->_check_cookie($cookie = $_COOKIE[$this->config->dungeon_cookie_name], $last_activity))
		{
			if (strtotime($this->config->dungeon_cookie_expire, $last_activity) < time())
			{
				$this->_session_id();
			}
			else
		...
		else if (!is_asset() && !$this->url->ajax && !$this->url->ajax_header && $_SERVER['REQUEST_METHOD'] != 'OPTIONS')
		{
			...
			if ($crawler !== FALSE)
			{
				$this->db->insert('dungeon_crawlers', [
					'name' => $crawler,
					'path' => $this->url->request
				]);
			}

			$this->db->insert('dungeon_sessions', [
				'session_id' => $this->_session_id,
				'ip_address' => $this->_ip_address,
				'host_name'  => $this->_host_name,
		...
		statistics('dungeon_sessions_max_simultaneous', $this->_sessions = $this->db->select('COUNT(DISTINCT IFNULL(user_id, session_id))')->from('dungeon_sessions')->where('last_activity > DATE_SUB(NOW(), INTERVAL 5 MINUTE)')->where('is_crawler', FALSE)->row(), function($a, $b){ return $a > $b; });
	}
	...
	{
		$this->_session_id = unique_id($this->db->select('session_id')->from('dungeon_sessions')->get());
		setcookie($this->config->dungeon_cookie_name, $this->_session_id, strtotime('+1 year'), url(), '', !empty($_SERVER['HTTPS']), TRUE);
	}
	...
	public function set_user_id($user_id)
	{
		...
		if ($user_id !== NULL)
		{
			$this->db->insert('dungeon_sessions_history', [
				'session_id'    => $this->_session_id,
				'user_id'       => $user_id,
				'ip_address'    => $this->_ip_address,
				'host_name'     => $this->_host_name,
