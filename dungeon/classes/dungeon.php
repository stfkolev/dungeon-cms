...
abstract class Dungeon
{
	...
	{
		if (($live_editor = post('live_editor')) && Dungeon()->user('admin'))
		{
			Dungeon()->session->set('live_editor', $live_editor);
			return $live_editor;
		}
	...
	{
		return isset($this->load->libraries[$name]) || isset(Dungeon()->libraries[$name]);
	}
	...
	public function __get($name)
	{
		...
		else if (isset(Dungeon()->libraries[$name]))
		{
			return Dungeon()->libraries[$name];
		}
		else
		{
			$type = 'libraries';
	...
	public function __call($name, $args)
	{
		...
		if (!$callback && is_callable($library = $this->$name ?: Dungeon()->$name))
		{
			$callback = $library;
		}
	...
	{
		Dungeon()->css[] = [$file, $media, $this->load];
		return $this;
	}

	public function js($file)
	{
		Dungeon()->js[] = [$file, $this->load];
		return $this;
	}

	public function js_load($function)
	{
		Dungeon()->js_load[] = $function;
		return $this;
	}

	public function module($name, $force = FALSE)
	{
		return $this->_load($name, 'module', 'm_'.$name, Dungeon()->modules, $name.'/'.$name, $force, function() use ($name){
	...
	{
		return $this->_load($name, 'theme', 't_'.$name, Dungeon()->themes, $name.'/'.$name, $force);
	}

	public function widget($name, $force = FALSE)
	{
		return $this->_load($name, 'widget', 'w_'.$name, Dungeon()->widgets, $name.'/'.$name, $force, function() use ($name){
	...
	{
		return $this->_load($name, 'authenticator', 'a_'.$name, Dungeon()->authenticators, NULL, FALSE, NULL, [$name, $enabled, $settings]);
	}
	...
	public function lang($name)
	{
		...
		foreach ($paths = $this->load->paths('lang') as $dir)
		{
			foreach ($this->config->langs as $language)
			{
				...
				else if (isset(Dungeon()->load->langs[$path]))
				{
					$lang = Dungeon()->load->langs[$path];
				}
				else
				{
					$lang = [];
	...
	{
		$output = Dungeon()	->label($title ?: (isset($this->name) ? $this->name : get_class($this)))
		...
		Dungeon()->debug->timeline($output, $this->time[0], $this->time[1]);
