<div class="debugbar hidden-xs<?php if ($data['active']) echo ' active'; ?>">
	...
	<nav>
		...
		<div class="pull-right">
			<p class="hidden-sm"><?php echo '<span class="badge">'.(post() ? 'POST' : 'GET').'</span> '.implode(' ', array_map(function($a){ return '<span class="badge">'.utf8_htmlentities($a).'</span>'; }, Dungeon()->router->segments)); ?></p>
			<p><?php echo icon('fa-clock-o').' '.round((microtime(TRUE) - DUNGEON_TIME) * 1000, 2).' ms'; ?></p>
			<p><?php echo icon('fa-cogs').' '.ceil((memory_get_peak_usage() - DUNGEON_MEMORY) / 1024).' kB'; ?></p>
