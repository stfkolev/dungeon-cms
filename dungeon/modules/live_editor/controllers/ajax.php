...
class m_live_editor_c_ajax extends Controller_Module
{
	public function zone_fork($disposition_id, $disposition, $url, $theme, $page, $zone)
	{
		...
		if ($page == '*' || !$this->db->select('1')->from('dungeon_dispositions')->where('page', $url)->row())
		{
			array_walk($disposition, function($c){
				$c->traversal(function($w){
					$w->widget_id($this->db->insert('dungeon_widgets', $this->db	->select('widget', 'type', 'title', 'settings')
																			->from('dungeon_widgets')
																			->where('widget_id', $w->widget_id())
																			->row()));
				});
			});

			return $this->zone($this->db->insert('dungeon_dispositions', [
				'theme'       => $theme,
				'page'        => $url,
				'zone'        => $zone,
	...
	{
		$widget_id = $this->db	->insert('dungeon_widgets', [
...
./dungeon/modules/live_editor/controllers/ajax.php
*/