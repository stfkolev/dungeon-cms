...
class m_monitoring_c_admin_ajax extends Controller_Module
{
	...
	public function index($refresh)
	{
		...
		{
			$this->config('dungeon_monitoring_last_check', time());
			...
			{
				if ($$file = $this	->network('https://dungeon.com/'.$file.'.json')
									->type('text')
									->get())
				{
			...
			$result = [
				'storage' => [
					'total'    => disk_total_space(DUNGEON_CMS) ?: 0,
					'free'     => disk_free_space(DUNGEON_CMS) ?: 0,
	...
	public function update()
	{
		$this->_stream(function(){
			...
			$this	->network('https://dungeon.download')
					->stream($file = 'cache/monitoring/dungeon.zip', function($size, $total){
			...
			if ($total = count($files))
			{
				...
				foreach (array_diff(array_keys(dir_scan('dungeon')), array_filter($files, function($a){
					return preg_match('_^dungeon/_', $a);
				})) as $file)
				{
					unlink($file);
				}

				$patchs = array_map('version_format', dir_scan('dungeon/install'));

				if (!$this->config->dungeon_version)
				{
					$this->config('dungeon_version', version_format(DUNGEON_VERSION));
				}

				$total = count($patchs = array_filter($patchs, function($a){
					return version_compare($a, $this->config->dungeon_version, '>');
				}));
				...
				$this	->config('dungeon_version', end($patchs))
						->config('dungeon_monitoring_last_check', 0);
			}
		});
	}
