<?php $groups = $this->groups(); if (!empty($groups)): ?>
	<form action="<?php echo url($this->url->request); ?>" method="post">
		...
		<?php foreach ($groups as $group_id => $group): ?>
			...
			<li class="col-md-4">
				<label>
					<input type="checkbox" name="<?php echo $data['form_id']; ?>[groups][]" value="<?php echo $group_id; ?>"<?php if (in_array($data['user_id'], $group['users'])) echo ' checked="checked"'; if ($group['auto'] && $group['auto'] != 'dungeon') echo ' disabled="disabled"'; ?> />
