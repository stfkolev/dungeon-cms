			...
			->add_columns([
				[
					'content' => function($data){
						return $data['auto'] != 'dungeon' ? $this->button_sort($data['data_id'], 'admin/ajax/user/groups/sort') : NULL;
					},
					'size'    => TRUE
				],
				[
					'content' => function($data){
						return Dungeon()->groups->display($data['data_id']);
					},
					'search'  => function($data){
						return Dungeon()->groups->display($data['data_id'], FALSE, FALSE);
					}
				],
				[
					'content' => function($data){
		...
		$table_users = $this
			...
			->add_columns([
				...
				[
					...
					'content' => function($data){
						return '<span data-toggle="tooltip" title="'.timetostr(Dungeon()->lang('date_time_long'), $data['registration_date']).'">'.time_span($data['registration_date']).'</span>';
					},
					'sort'    => function($data){
				...
				[
					...
					'content' => function($data){
						return '<span data-toggle="tooltip" title="'.timetostr(Dungeon()->lang('date_time_long'), $data['last_activity_date']).'">'.time_span($data['last_activity_date']).'</span>';
					},
					'sort'    => function($data){
			...
			->add_rules([
				'groups' => [
					...
					'values' => array_filter($this->groups(), function($group){
						return !$group['auto'] || $group['auto'] == 'dungeon' || $group['users'] !== NULL;
					}),
					'rules'  => 'required'
				]
			])
	...
	public function delete($user_id, $username)
	{
		...
		if ($this->form->is_valid())
		{
			$this->db	->where('user_id', $user_id)
						->update('dungeon_users', ['deleted' => TRUE]);
		...
		$this	->title($this->lang('sessions'))
				...
				->add_columns([
					...
					[
						...
						'content' => function($data){
							return $data['user_id'] ? Dungeon()->user->link($data['user_id'], $data['username']) : '<i>'.$this->lang('guest').'</i>';
						},
						'search'  => function($data){
					...
					[
						'content' => [function($data){
							if ($data['user_id'] && $data['session_id'] != Dungeon()->session('session_id'))
							{
								return $this->button_delete('admin/user/sessions/delete/'.$data['session_id']);
							}
						}]
...
/*
Dungeon Alpha 0.1.6
./dungeon/modules/user/controllers/admin.php
*/