...
class m_user_c_index extends Controller_Module
{
		...
		$active_sessions = $this->table
			->add_columns([
				...
				[
					...
					'content' => function($data){
						return '<span data-toggle="tooltip" title="'.timetostr(Dungeon()->lang('date_time_long'), $data['date']).'">'.time_span($data['date']).'</span>';
					}
				],
				[
					'title'   => $this->lang('last_activity'),
					'content' => function($data){
						return '<span data-toggle="tooltip" title="'.timetostr(Dungeon()->lang('date_time_long'), $data['last_activity']).'">'.time_span($data['last_activity']).'</span>';
					}
				],
				[
					'content' => [function($data){
						if ($data['session_id'] != Dungeon()->session('session_id'))
						{
	...
	public function _auth($authenticator)
	{
		try
		{
			...
			$service = new \SocialConnect\Auth\Service(
				...
				new \SocialConnect\Provider\Session\Dungeon($this->session), [
			...
			if ($callback = $authenticator->data($params))
			{
				...
				if (!($user_id = $this->db->select('user_id')->from('dungeon_users_auth')->where('authenticator', $authenticator->name)->where('id', $data['id'])->row()))
				{
					if ($data['avatar'])
					{
					...
					while ($data['username'] && $this->db->select('1')->from('dungeon_users')->where('username', $data['username'])->row())
					{
						$data['username'] = 'guest'.time();
					}

					if ($data['language'] && !$this->db->select('1')->from('dungeon_settings_languages')->where('code', $data['language'])->row())
					{
						$data['language'] = '';
					}

					if ($data['email'] && (!is_valid_email($data['email']) || $this->db->select('1')->from('dungeon_users')->where('email', $data['email'])->row()))
					{
						$data['email'] = '';
					}
					...
					$user_id = $this->db->insert('dungeon_users', [
					...
					$this->db->insert('dungeon_users_auth', [
						'user_id'       => $user_id,
					...
					{
						$this->db->insert('dungeon_users_profiles', [
							'user_id'       => $user_id,
	...
	public function login($error = 0)
	{
		...
		$rules = [
			'username' => [
				...
				'check' => function($value){
					if (Dungeon()->db->select('1')->from('dungeon_users')->where('username', $value)->row())
					{
			...
			'email' => [
				...
				'rules' => 'required',
				'check' => function($value){
					if (Dungeon()->db->select('1')->from('dungeon_users')->where('email', $value)->row())
					{
		...
		if (in_array($error, [Dungeon::UNCONNECTED, Dungeon::UNAUTHORIZED]))
		{
		...
		else if ($form_registration->is_valid($post) && $this->config->dungeon_registration_status == 0)
		{
			$user_id = $this->db->insert('dungeon_users', [
			...
			if ($this->config->dungeon_welcome && $this->config->dungeon_welcome_user_id && $this->config->dungeon_welcome_title && $this->config->dungeon_welcome_content)
			{
				$this->model('messages')->insert_message($post['username'], $this->config->dungeon_welcome_title, str_replace('[pseudo]', '@'.$post['username'], $this->config->dungeon_welcome_content), TRUE);
			}
		...
		if ($this->form->is_valid($post))
		{
			$this->email
				...
				->message('default', [
					...
					'key'     => $this->model()->add_key($this->db->select('user_id')->from('dungeon_users')->where('email', $post['email'])->row())
				])
				->send();
	...
	public function _lost_password($key_id, $user_id)
	{
		...
		{
			$this->email
				->to($this->db->select('email')->from('dungeon_users')->where('user_id', $user_id)->row())
	...
	public function logout()
	{
		...
		if ($this->config->dungeon_http_authentication)
		{
			$this->ajax();
