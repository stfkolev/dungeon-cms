...
class m_user_m_user extends Model
{
	...
	public function edit_user($username, $email, $first_name, $last_name, $avatar, $date_of_birth, $sex, $location, $website, $quote, $signature, $user_id = NULL)
	{
		...
		if ($this->db->select('1')->from('dungeon_users_profiles')->where('user_id', $user_id)->row())
		{
			$this->db	->where('user_id', $user_id)
						->update('dungeon_users_profiles', $data);
		}
		else
		{
			$this->db->insert('dungeon_users_profiles', array_merge($data, [
				'user_id' => $user_id
			]));
		}
	...
	public function edit_groups($user_id, $groups)
	{
		...
		foreach ($groups as $group_id)
		{
			...
			$this->db->insert('dungeon_users_groups', [
				'user_id'  => $user_id,
				'group_id' => $group_id
			]);
		}
	...
	{
		$this->db->insert('dungeon_users_keys', [
			'key_id'     => $key_id = unique_id($this->db->select('key_id')->from('dungeon_users_keys')->get()),
			'user_id'    => $user_id,
	...
	public function check_session($session_id)
	{
		return $this->db	->select('s.session_id', 'u.username')
							...
							->join('dungeon_users u', 'u.user_id = s.user_id')
	...
	public function get_sessions()
	{
		return $this->db->select('u.user_id', 'u.username', 's.session_id', 's.ip_address', 's.host_name', 's.last_activity', 's.user_data', 's.remember_me')
						->from('dungeon_sessions s')
						->join('dungeon_users u', 'u.user_id = s.user_id AND u.deleted = "0"')
...
/*
Dungeon Alpha 0.1.6
./dungeon/modules/user/models/user.php
*/